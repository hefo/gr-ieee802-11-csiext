...
using namespace gr::ieee802_11_csiext::equalizer;
...
std::string comb::get_csi() {
	...
	for(int i = 0; i < 64; i++) {
		...
		d_H_real = std::to_string(d_H[i].real());
		d_H_imag = std::to_string(d_H[i].imag());
			
		std::replace(d_H_real.begin(), d_H_real.end(), ',', '.');
		std::replace(d_H_imag.begin(), d_H_imag.end(), ',', '.');
		
		if(d_H_imag.find('-') == std::string::npos){
			d_H_imag = "+" + d_H_imag;
		}
		d_csi.append(d_H_real+d_H_imag+"i,");
		
		
		
	}
	d_csi.erase(d_csi.size() - 1);
